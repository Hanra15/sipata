...
class M_fasum extends CI_Model
{
    ...
    public function get_datatables($postData = null)
    {
        ...
        $result = guzzle_request('GET', 'fasum?token=' . $token, []);
        ...
        foreach ($result["data"] as $field) {
            ...
            // $row ['id']	=  $field['id']; 
            $row['No']      =  $no++;
            $row['Nama Fasilitas Umum']    = $field['name_fasum'];
            $row['Kategori Fasilitas Umum']   = $field['category_fasum']['name_category'];
            $row['Alamat']    = $field['address'];
            $row['Kontak']    = $field['contact_fasum'];
            $row['aksi'] = '
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item text-primary" href="' . base_url() . 'fasum/Fasum/detail_fasum/' . $field['id'] . '"><i class="bi bi-eye-fill"></i> Detail</a></li>
                        <li><a class="dropdown-item text-success" href="' . base_url() . 'fasum/Fasum/edit_fasum/' . $field['id'] . '"><i class="bi bi-pencil-square"></i> Edit</a></li>
